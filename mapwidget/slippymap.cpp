...
{
    const qreal zn = static_cast<qreal>(1 << zoom);
    const qreal tx = (lng + 180.0) / 360.0;
    const qreal ty = (1.0 - log(tan(lat * M_PI / 180.0) + 1.0 / cos(lat * M_PI / 180.0)) / M_PI) / 2.0;
...
{
    const qreal zn = static_cast<qreal>(1 << zoom);
    const qreal lat = tx / zn * 360.0 - 180.0;
    return lat;
}
...
{
    const qreal zn = static_cast<qreal>(1 << zoom);
    const qreal n = M_PI - 2 * M_PI * ty / zn;
    const qreal lng = 180.0 / M_PI * atan(0.5 * (exp(n) - exp(-n)));
    return lng;
}

SlippyMap::SlippyMap(QObject *parent)
    : QObject(parent), width(400), height(300), zoom(13),
